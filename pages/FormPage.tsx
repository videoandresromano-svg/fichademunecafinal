...
const FormPage: React.FC<{ onFormSubmit: () => void; initialData?: ClinicalData }> = ({ onFormSubmit, initialData }) => {
    ...
    const handleRemoveImage = (indexToRemove: number) => {
        const imageToRemove = formData.radiology.studies[indexToRemove];
        if (imageToRemove?.id) {
            deleteImage(imageToRemove.id).catch(err => {
        ...
        switch (STEPS[currentStep].id) {
            ...
            case 'exam':
                const goniometriaMovements = [
                 ...
                 const CRPSCheckboxGroup: React.FC<{title: string, groupKey: keyof PhysicalExamData['riesgoCapsulitisAdhesiva']['evaluacionCRPS'], items: {key: string, label: string}[]}> = ({ title, groupKey, items }) => (
                    <div>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-2">
                            {items.map(item => (
                                <Checkbox
                                    key={item.key}
                                    id={`crps-${groupKey}-${item.key}`}
                                    label={item.label}
                                    name={`riesgoCapsulitisAdhesiva.evaluacionCRPS.${groupKey}.${item.key}`}
                                    // @ts-ignore
                                    checked={formData.physicalExam.riesgoCapsulitisAdhesiva.evaluacionCRPS[groupKey][item.key]}
                            ...
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div className="lg:col-span-3">
                                 ...
                                 <div className="md:col-span-2 lg:col-span-3">
                                    <label className="block text-sm font-medium text-slate-700">Evaluación de CRPS (Criterios de Budapest)</label>
                                    <div className="mt-2 space-y-4 p-3 bg-slate-50 rounded-md border">
                                        <CRPSCheckboxGroup 
                                            title="1. Dolor y Alteraciones Sensitivas"
                                            groupKey="dolorAlteracionesSensitivas"
                                            items={[
                                                {key: 'dolorDesproporcionado', label: 'Dolor desproporcionado (ardoroso, etc.)'},
                                                {key: 'alodinia', label: 'Alodinia'},
                                                {key: 'hiperalgesia', label: 'Hiperalgesia'},
                                                {key: 'parestesiasDisestesias', label: 'Parestesias / Disestesias'}
                                            ]}
                                        />
                                        <CRPSCheckboxGroup 
                                            title="2. Alteraciones Autonómicas y Tróficas"
                                            groupKey="alteracionesAutonomicasTroficas"
                                            items={[
                                                {key: 'cambiosVasomotores', label: 'Cambios vasomotores (color, temp.)'},
                                                {key: 'cambiosSudomotores', label: 'Cambios sudomotores (sudor)'},
                                                {key: 'edemaPersistente', label: 'Edema persistente'},
                                                {key: 'cambiosTroficosPiel', label: 'Cambios tróficos (piel, vello, uñas)'},
                                                {key: 'osteopeniaLocalizada', label: 'Osteopenia localizada (tardío)'}
                                            ]}
                                        />
                                        <CRPSCheckboxGroup 
                                            title="3. Alteraciones Motoras y Funcionales"
                                            groupKey="alteracionesMotorasFuncionales"
                                            items={[
                                                {key: 'rigidezArticular', label: 'Rigidez articular'},
                                                {key: 'debilidadMuscular', label: 'Debilidad muscular'},
                                                {key: 'tembloresDistonia', label: 'Temblores / Distonía'},
                                                {key: 'limitacionFuncionalProgresiva', label: 'Limitación funcional progresiva'}
                                            ]}
                                        />
                                        <CRPSCheckboxGroup 
                                            title="➕ Factores de Personalidad Proclives"
                                            groupKey="factoresPersonalidad"
                                            items={[
                                                {key: 'ansiedadHipervigilancia', label: 'Ansiedad / Hipervigilancia'},
                                                {key: 'rasgosDepresivos', label: 'Rasgos depresivos'},
                                                {key: 'catastrofizacionDolor', label: 'Catastrofización del dolor'},
                                                {key: 'perfeccionismoAutoexigencia', label: 'Perfeccionismo / Autoexigencia'},
                                                {key: 'afrontamientoPasivo', label: 'Afrontamiento pasivo'},
                                                {key: 'estresPsicosocialCronico', label: 'Estrés psicosocial crónico'}
                                            ]}
                                        />
                                    </div>
                                </div>
                    ...
                    <div className="flex flex-col gap-8 items-center pt-4">
                        ...
                        {summary && (
                            <div className="w-full mt-4 bg-slate-50 p-4 rounded-lg border animate-fadeIn">
                                <label className="block text-sm font-medium text-slate-700 mb-1">Resumen y Análisis</label>
                                <div className="mt-1 w-full p-4 bg-slate-800 rounded-md border border-slate-700 min-h-[300px] text-sm overflow-y-auto">
