...
const formatDataForCIFPrompt = (record: ClinicalRecord, patient: Patient): string => {
    const { anamnesis, physicalExam, scales, radiology, summary, impact } = record;
    ...
    // --- Scales ---
    const calculateDashScore = (items: {[key: string]: string}, itemKeys: string[]) => {
        ...
        if (itemKeys.length === 30 && n < 27) return null; // Main DASH module requires at least 27 items
        const score = ((sum / n) - 1) * 25;
        return Math.round(score * 100) / 100;
    };

    const mainDashKeys = Array.from({ length: 30 }, (_, i) => `item${i + 1}`);
    const mainDashScore = calculateDashScore(scales.dash.items, mainDashKeys);
    let dashScoreString = mainDashScore !== null ? `Puntuación DASH Global: ${mainDashScore}/100.` : 'No medido con DASH.';

    const { dolor, funcion } = scales.prwe;
    const parsePRWE = (val: string) => parseInt(val, 10) || 0;
    const totalDolor = Object.values(dolor).reduce((sum, val) => sum + parsePRWE(val), 0);
    const totalFuncion = Object.values(funcion).reduce((sum, val) => sum + parsePRWE(val), 0) / 2;
    const prweGlobal = totalDolor + totalFuncion;
    const prweScoreString = `Puntuación PRWE Global: ${prweGlobal}/100 (Dolor: ${totalDolor}/50, Función: ${totalFuncion}/50).`;

    // --- Physical Exam ---
    const gonioString = Object.entries(physicalExam.goniometria)
        .map(([key, value]) => {
            const formattedValue = formatBilateralValue(value);
            return formattedValue ? `${key.charAt(0).toUpperCase() + key.slice(1)}: ${formattedValue}` : null;
        })
        .filter(Boolean).join('; ');

    const prensionPositivas = Object.entries(physicalExam.pruebasPrension)
        .filter(([, checked]) => checked)
        .map(([key]) => key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()))
        .join(', ');

    // --- Impact / Flags ---
    const flagsData = impact ? [
        (impact.roja_dolor_golpe === 'si' || impact.roja_fiebre_inflamacion === 'si') && 'Banderas Rojas (posible fractura/infección)',
        (impact.amarilla_creencia_dolor === 'empeorara' || impact.amarilla_evita_actividades === 'si') && 'Banderas Amarillas (creencias/miedo)',
        (impact.azul_trabajo_dificulta === 'si' || impact.azul_apoyo_laboral === 'no') && 'Banderas Azules (factores laborales)',
        (impact.negra_barreras_recuperacion === 'si') && 'Banderas Negras (barreras contextuales)',
        (impact.naranja_animo_interes === 'si' || impact.naranja_desesperanza === 'si') && 'Banderas Naranjas (salud mental)',
    ].filter(Boolean).join('; ') : 'No informado';

    const dataPoints = `
**Datos Demográficos y Contextuales:**
- Edad: ${filiatorios.edad}
- Dominancia: ${anamnesis.dominancia}
- Ocupación: ${filiatorios.ocupacion}
- Actividades/Deportes: ${filiatorios.actividadesActuales || 'N/A'} / ${filiatorios.deportesActuales || 'N/A'}
- Factores de riesgo: ${[anamnesis.tabaquismo === 'si' && 'Tabaquismo', anamnesis.caidasFrecuentes === 'si' && `Caídas frecuentes (${anamnesis.caidas6meses})`].filter(Boolean).join(', ') || 'Ninguno'}
- Comorbilidades: ${[anamnesis.diabetes === 'si' && 'Diabetes', anamnesis.osteopeniaOsteoporosis === 'si' && 'Osteoporosis/penia', anamnesis.dsr === 'si' && 'SDRC previo'].filter(Boolean).join(', ') || 'Ninguna'}
- Medicación Relevante: ${anamnesis.medicacionExtra || 'Ninguna'}

**Funciones Corporales:**
- Sensación de Dolor (EVA): ${scales.dolorVAS}/10. Nocturno: ${scales.dolorNocturnoSeveridad}/10. Frecuencia diurna: ${scales.dolorDiurnoFrecuencia}/10.
- Sensaciones Neuropáticas (Hormigueo/Entumecimiento): ${scales.hormigueo}/10.
- Fuerza Muscular (percepción de debilidad): ${scales.debilidad}/10.
- Estabilidad Articular: ${physicalExam.cftLesion.sensacionInestabilidad === 'si' ? 'Refiere inestabilidad' : 'No refiere inestabilidad'}.
- Movilidad Articular (Goniometría): ${gonioString || 'No medida'}.

**Estructuras Corporales:**
- Hallazgos Radiológicos: ${radiology.interpretation || 'Sin interpretación'}. Varianza Cubital: ${radiology.varianzaCubital || 'N/A'}${radiology.varianzaCubitalClasificacion ? ` (${radiology.varianzaCubitalClasificacion})` : ''}.
- Edema (Figura en 8): ${physicalExam.medidas.figuraEn8} cm.
- Actitud del Miembro Superior: Hombro: ${physicalExam.actitudMiembroSuperior.hombro}, Codo: ${physicalExam.actitudMiembroSuperior.codoAntebrazo}, Muñeca: ${physicalExam.actitudMiembroSuperior.muneca}, Mano: ${physicalExam.actitudMiembroSuperior.mano}.

**Actividad y Participación:**
- Dificultad general (PRWE): ${prweScoreString}
- Dificultad en AVDs (DASH): ${dashScoreString}
- Dificultad para levantar y llevar objetos: Reflejado en PRWE/DASH.
- Dificultad en tareas del hogar y trabajo: Reflejado en PRWE/DASH.
- Tipos de Prensión Afectados: ${prensionPositivas || 'No especificado'}.
- Test de Kapandji (Oposición pulgar): ${formatBilateralValue(physicalExam.testKapandji, '/10') || 'No medido'}.
- Movilidad funcional (TUG Test): ${scales.tugTest ? `${scales.tugTest} segs` : 'No medido'}.

**Factores Ambientales y Personales:**
- Banderas Biopsicosociales: ${flagsData || 'No informado'}.
- Interpretación del Profesional: ${impact?.interpretacionProfesional || 'No informada'}.
- Apoyo laboral: ${impact?.azul_apoyo_laboral ? (impact.azul_apoyo_laboral === 'si' ? 'Presente' : 'Ausente') : 'No informado'}.
- Apoyo del entorno: ${impact?.negra_apoyo_entorno ? formatDisplayValue(impact.negra_apoyo_entorno) : 'No informado'}.
- Barreras del sistema: ${impact?.negra_barreras_sistema === 'si' ? 'Presentes' : 'No reportadas'}.
- Contexto de vida y hogar: ${filiatorios.domicilio}, ${filiatorios.localidad}.

**Resumen Clínico General:**
${summary}
`;
    return dataPoints.trim();
};
...
Tu tarea es analizar la información detallada de un paciente y generar un perfil CIF preciso y clínicamente relevante.

**Regla de Oro: ¡NUNCA INVENTES CÓDIGOS!** Usa solo códigos que existen en la clasificación oficial de la CIF (generalmente una letra y 3 dígitos, ej. b280, s730, d430).

**Instrucciones Clave:**
1.  **Analiza exhaustivamente TODOS los datos proporcionados**: Demográficos, anamnesis, funciones, estructuras, escalas funcionales (PRWE/DASH), y factores contextuales (banderas).
2.  **Codificación Estricta y Precisa**:
    *   Utiliza **únicamente** códigos CIF estándar. **Está estrictamente prohibido inventar códigos de sub-nivel o de 4 dígitos (como \`b7100\` o \`b2801\`).**
    *   La **especificidad** (como 'muñeca derecha', 'flexión', 'hormigueo') **DEBE** ir en el campo de \`descripcion\`, no en el código.
    *   Si un concepto específico no tiene un código propio (ej. "hormigueo"), busca la categoría general más cercana (ej. "b265 Funciones táctiles") y añade un \`.8\` al final para indicar 'otras especificadas', detallando en la descripción.
3.  **Calificadores Basados en Evidencia**: Asigna a cada código un calificador numérico de deficiencia/dificultad (0=ninguna, 1=leve, 2=moderada, 3=grave, 4=completa) basado en la información clínica. Por ejemplo, un EVA de 7/10 es una deficiencia 'grave' (3). Una limitación del 50% en ROM es 'moderada' (2).
4.  **Factores Ambientales**: Usa calificadores de barrera (ej. "2") o facilitador (ej. "+2"). Por ejemplo, un buen apoyo familiar es un facilitador. La falta de apoyo laboral es una barrera.
5.  **Factores Personales**: Describe cualitativamente los factores personales relevantes (ej. alta autoexigencia, miedo al movimiento, ocupación de riesgo).
6.  **Formato JSON Estricto**: La salida debe ser exclusivamente un JSON válido con la estructura solicitada.

**Ejemplos de Codificación Correcta e Incorrecta:**
- **CORRECTO (Dolor EVA 7/10):** { "codigo": "b280", "descripcion": "Sensación de dolor (muñeca derecha, diurno y nocturno)", "calificador": "3" }
- **CORRECTO (Hormigueo):** { "codigo": "b265.8", "descripcion": "Funciones táctiles, otras especificadas (sensación de hormigueo en muñeca derecha)", "calificador": "2" }
- **INCORRECTO (Hormigueo):** { "codigo": "b2801", "descripcion": "Sensación de hormigueo", "calificador": "2" }  <-- ¡CÓDIGO INCORRECTO Y PROHIBIDO!
- **CORRECTO (Limitación severa de flexión):** { "codigo": "b710", "descripcion": "Movilidad de una articulación (muñeca derecha, flexión)", "calificador": "3" }
- **INCORRECTO (Limitación severa de flexión):** { "codigo": "b7100", "descripcion": "Movilidad de flexión de muñeca", "calificador": "3" } <-- ¡CÓDIGO INCORRECTO Y PROHIBIDO!
- **CORRECTO (Dificultad para levantar objetos):** { "codigo": "d430", "descripcion": "Levantar y llevar objetos", "calificador": "3" }
- **CORRECTO (Trabajo como barrera):** { "codigo": "e590", "descripcion": "Servicios, sistemas y políticas laborales y de empleo", "calificador": "2" }
- **CORRECTO (Buen apoyo familiar):** { "codigo": "e310", "descripcion": "Familiares cercanos", "calificador": "+3" }

---
**DATOS DEL PACIENTE PARA ANÁLISIS:**
