import { CIFProfile, FiliatoriosData, AnamnesisData, PhysicalExamData, ScalesData, RadiologyData, HypothesisComparison, ImpactData, StudyImage } from '../types';
// @ts-ignore
import initSqlJs from 'sql.js';
import { addImage, getImage } from './imageStore';
    ...
    if (dbInstance) {
        try {
            ...
            if (e instanceof DOMException && (e.name === 'QuotaExceededError' || e.code === 22)) {
                 console.error("LocalStorage quota exceeded. Cannot save database.", e);
                 alert("Error: No se pudo guardar la base de datos. El almacenamiento local está lleno. Esto puede deberse a imágenes muy grandes. Considere exportar y limpiar datos antiguos.");
            } else {
                console.error("Failed to save database:", e);
                 alert(`Error al guardar la base de datos: ${e.message}`);
            }
        }
    }
};
...
export const getPatientDetails = async (patientId: string) => {
    ...
    while (recordsStmt.step()) {
        ...
        const radiologyData = JSON.parse(row.radiology_data as string || '{}');
        if (radiologyData.studies && radiologyData.studies.length > 0) {
            const studiesWithImages = await Promise.all(
                radiologyData.studies.map(async (study: StudyImage) => {
                    const imageData = await getImage(study.id);
                    return { ...study, base64: imageData?.base64 || undefined };
                })
            );
            radiologyData.studies = studiesWithImages;
        }
...
export const saveClinicalRecord = async (data: {
    ...
    const radiologyDataForDb = JSON.parse(JSON.stringify(data.radiology));
    if (radiologyDataForDb.studies && radiologyDataForDb.studies.length > 0) {
        const imagePromises = radiologyDataForDb.studies.map(async (study: StudyImage) => {
            if (study.base64) {
                await addImage(study.id, study.base64);
                delete study.base64; 
            }
        });
    ...
    recordStmt.run([
        ...
        JSON.stringify(radiologyDataForDb),
